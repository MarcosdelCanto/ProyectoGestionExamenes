  ...
  try {
    ...
    if (userRole === 'ALUMNO') {
      ...
      sql = `
        ...
        JOIN ESCUELA ESC ON C.ESCUELA_ID_ESCUELA = ESC.ID_ESCUELA
        JOIN SALA S ON R.SALA_ID_SALA = S.ID_SALA
        JOIN USUARIOSECCION US ON SEC.ID_SECCION = US.SECCION_ID_SECCION -- JOIN para alumnos con USUARIOSECCION
        WHERE R.ESTADO_CONFIRMACION_DOCENTE = 'CONFIRMADO'
        AND US.USUARIO_ID_USUARIO = :userId -- Filtrar por las secciones del alumno
        ORDER BY R.FECHA_RESERVA
      `;
    } else {
      // Para docentes y otros roles: mostrar reservas donde están asignados como docentes
      sql = `
        SELECT DISTINCT
          R.ID_RESERVA, R.FECHA_RESERVA,
          S.ID_SALA, S.NOMBRE_SALA,
        ...
        JOIN ESCUELA ESC ON C.ESCUELA_ID_ESCUELA = ESC.ID_ESCUELA
        JOIN SALA S ON R.SALA_ID_SALA = S.ID_SALA
        JOIN RESERVA_DOCENTES RD ON R.ID_RESERVA = RD.RESERVA_ID_RESERVA -- JOIN para docentes con RESERVA_DOCENTES
        WHERE R.ESTADO_CONFIRMACION_DOCENTE = 'CONFIRMADO'
        AND RD.USUARIO_ID_USUARIO = :userId -- Filtrar por el docente asignado a la reserva
        ORDER BY R.FECHA_RESERVA
      `;
    }

...
export const consultarReservasPublico = async (req, res) => {
  ...
  try {
    ...
    if (usuario.ROL_ID_ROL === ID_ROL_ALUMNO) {
      ...
      // y LEFT JOIN para obtener el nombre del docente
      sqlReservas = `
        SELECT
        ...
        LEFT JOIN RESERVA_DOCENTES RD ON R.ID_RESERVA = RD.RESERVA_ID_RESERVA
        LEFT JOIN USUARIO U_DOC ON RD.USUARIO_ID_USUARIO = U_DOC.ID_USUARIO AND U_DOC.ROL_ID_ROL = 2
      ...
      // y JOIN para obtener su propio nombre
      sqlReservas = `
        SELECT
          R.ID_RESERVA, R.FECHA_RESERVA,
          E.NOMBRE_EXAMEN,
        ...
        JOIN USUARIO U_DOC ON RD.USUARIO_ID_USUARIO = U_DOC.ID_USUARIO AND U_DOC.ROL_ID_ROL = 2
